    ...
    private fun detectLocalModel() {
        get<AppScope>().launch(Dispatchers.IO) {
            ...
            runCatching {
                ...
                val ports = mapOf(
                    18888 to "model-router",
                    6791 to "hermes-proxy",
                )
                ...
                for ((port, _) in ports) {
                    for (host in hosts) {
                        ...
                        if (ok) {
                            found[port] = host
                            Log.i(TAG, "detectLocalModel: $port 在 $host 上响应")
                            break
                        }
                    }
                }
                ...
                if (found.isEmpty()) {
                    Log.d(TAG, "detectLocalModel: 未找到本地/远程服务")
                    return@launch
                }

                Log.i(TAG, "detectLocalModel: 发现 ${found.size} 个服务")

                store.update { s ->
                    var providers = s.providers.toMutableList()

                    val modelHost = found[18888]
                    if (modelHost != null) {
                        try {
                            val modelsUrl = "http://$modelHost:18888/v1/models"
                            ...
                            val localProvider = ProviderSetting.OpenAI(
                                ...
                                name = "本地模型",
                                baseUrl = "http://$modelHost:18888/v1",
                                apiKey = "not-needed",
                                enabled = true,
                                builtIn = true,
                                models = modelList,
                    ...
                    if (hermesHost != null) {
                        ...
                        val hermesProvider = ProviderSetting.OpenAI(
                            id = hermesId,
                            name = "Hermes 助手",
                            baseUrl = "http://$hermesHost:6791/v1",
                            apiKey = "not-needed",
                            enabled = true,
                            builtIn = true,
                            models = listOf(Model(
