    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                webServerPort = preferences[WEB_SERVER_PORT] ?: 8787,
        ...
        .map {
            ...
            // 第1步：删除指向已删除的 sse-proxy (:6791) 的残留 provider
            providers = providers.filter { provider ->
                val url = when (provider) {
                    is ProviderSetting.OpenAI -> provider.baseUrl
                    else -> ""
                }
                val keep = !url.contains(":6791")
                if (!keep) {
                    Log.w(TAG, "Removing stale provider '${provider.name}' at $url (sse-proxy deleted)")
                }
                keep
            }.toMutableList()

            // 第2步：按 ID 匹配注入/同步默认 provider（包括 baseUrl）
            providers = providers.map { provider ->
                val defaultProvider = DEFAULT_PROVIDERS.find { it.id == provider.id }
                if (defaultProvider != null) {
                    var p = provider.copyProvider(
                        builtIn = defaultProvider.builtIn,
                        description = defaultProvider.description,
                        shortDescription = defaultProvider.shortDescription,
                    )
                    if (p is ProviderSetting.OpenAI && defaultProvider is ProviderSetting.OpenAI) {
                        p = p.copy(baseUrl = defaultProvider.baseUrl, apiKey = defaultProvider.apiKey)
                    }
                    p
                } else provider
            }.toMutableList()

            // 第3步：按名称匹配强制覆写 baseUrl（处理旧版 UUID 残留）
            val hermesDefault = DEFAULT_PROVIDERS.find { it.id == DEFAULT_HERMES_ID }
            providers = providers.map { provider ->
                if (provider.name == "Hermes 助手" && provider.id != DEFAULT_HERMES_ID) {
                    Log.w(TAG, "Forcing baseUrl for provider '${provider.name}' (id=${provider.id})")
                    if (provider is ProviderSetting.OpenAI && hermesDefault is ProviderSetting.OpenAI) {
                        provider.copy(baseUrl = hermesDefault.baseUrl, apiKey = hermesDefault.apiKey)
                    } else provider
                } else provider
            }.toMutableList()
            ...
            // 自动绑定默认 MCP 服务器到默认助手
            val defaultMcpIds = DEFAULT_MCP_SERVERS.map { it.id }.toSet()
            if (defaultMcpIds.isNotEmpty()) {
                val idx = assistants.indexOfFirst { it.id == DEFAULT_ASSISTANT_ID }
                if (idx >= 0) {
                    assistants[idx] = assistants[idx].copy(
                        mcpServers = assistants[idx].mcpServers + defaultMcpIds
                    )
                }
            }
            ...
            it.copy(
                ...
                mcpServers = it.mcpServers.let { servers ->
                    val mutable = servers.toMutableList()
                    // 清理同名旧条目（ID 不同但 name 相同的残留）
                    val defaultNames = DEFAULT_MCP_SERVERS.map { it.commonOptions.name }.toSet()
                    mutable.removeAll { existing ->
                        existing.commonOptions.name in defaultNames &&
                        DEFAULT_MCP_SERVERS.none { it.id == existing.id }
                    }
                    DEFAULT_MCP_SERVERS.forEach { defaultMcp ->
                        if (mutable.none { it.id == defaultMcp.id }) {
                            mutable.add(defaultMcp.clone())
                        }
                    }
                    mutable
                },
        ...
        .map { settings ->
            ...
            settings.copy(
                ...
                mcpServers = settings.mcpServers.distinctBy { it.id },
